...
\begin{center}
    ...
    \end{figure}
\end{center}
The code pseudocode of each MCU consists in a setup function, called only once when the devices are turned on or after a reset and another that is continuously running and it is instead the core of the implementation.
...
%The Syntiant has an Arduino inside it, so the code uploading is performed through Arduino IDE. 

%\begin{algorithm}[B]
%\caption{Pseudocode Hardware Integration}
%\KwIn{Microphone audio stream}
%\KwOut{Result of the model (binary)}
    
%\nl \textbf{System Initialization:}\;
%\Indp
%    Load NDP with the binary executable obtained by Edge Impulse or Syntiant Quantization Tool;
%    Set up microphone on and as input source\;
%   Set up interrupt handler for output classification\;
%    Initialize low-power/sleep mode parameters, to reduce power consumption\;
%\Indm
%
%\nl \textbf{Main Processing Loop:}\;
%\Indp
%    \While{system running}{
%        \textbf{Enter Low-Power State:}\;
%        \Indp
%            Disable ADC and serial peripherals\;
%            Configure deep sleep mode\;
%            Execute Wait-For-Interrupt (WFI) instruction\;
%        \Indm
        
%        \textbf{On Wake Event:}\;
%        \Indp
%            Re-enable system peripherals\;
%        \Indm
%    }
%\Indm

%\nl \textbf{Interrupt Handler (NDP\_ISR):}\;
%\Indp
%    Triggered when NDP detects audio pattern\;
%    Using NDP.poll() to get model output (SV requires modification, because function output is single)\;
%    Classification matching or cosine similarity computation\;
%\Indm

%\nl \textbf{Output Service Routine:}\;
%\Indp
%    At each loop, if a sound was detected it performs a hardware reaction, else does nothing\;
%    Customizable part by the user\;
%\Indm

%\KwRet{Classification result to application layer to be used by the upper system}
%\end{algorithm}
\newpage
